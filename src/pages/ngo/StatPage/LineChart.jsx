...
const LineChart = ({ data }) => {
	...
	const convertDataForLineChart = (data) => {
		...
		const minDate = new Date(
			Math.min(...data.map((report) => new Date(report.reported_time)))
		);
		// End at the maximum date in the data
		const maxDate = new Date(
			Math.max(...data.map((report) => new Date(report.reported_time)))
		);
	...
	return (
		<div>
			...
			<div className="w-ful px-6 pt-3 flex justify-between mb-2 text-sm md:text-md">
				<button
					onClick={() => setView("day")}
					style={{
						backgroundColor: view === "day" ? "lightgray" : "white",
					}}
			...
			<Line
				...
				options={{
					...
					elements: {
						// Specify line styles under the 'elements' key
						line: {
							borderColor: "red",
							borderWidth: 3,
